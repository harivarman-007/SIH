...
export const CorporateManagementView: React.FC<CorporateManagementViewProps> = ({
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.error('Failed to load live corporate telemetry:', err);
      setError(err?.message || 'Failed to fetch cross-mine corporate data from http://localhost:8000/kpi/cross-mine-summary');
      setData(null);
    } finally {
      setIsLoading(false);
    }
